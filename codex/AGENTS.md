...
- `/ai-global` skill（`check`／`sync`／`deploy`／`govcheck`／`capabilities`／`install <name>`）是 Claude Code 的，**Codex 這側跑不了**：請使用者去 Claude Code 執行，或照 `<repo>/README.md` 的「給 AI agent 的指引」手動走。
