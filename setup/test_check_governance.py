...
class GovernanceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.files = {}
        body = "\n".join(f"- `~/Developer/agent-governance/{name}`" for name in checker.GOVERNANCE_FILES)
        for name in checker.ROUTERS:
            self.files[self.root / name] = f"# {name}\n{body}\n"
    ...
    def test_router_drift_fails(self):
        self.files[self.root / checker.ROUTERS[0]] += "不同規則\n"
        self.assertEqual(self.run_repository(), 1)
        self.assertIn("內容不一致", self.output())

    def test_missing_route_even_in_fenced_example_fails(self):
        for name in checker.ROUTERS:
            path = self.root / name
            self.files[path] = self.files[path].replace(
                "- `~/Developer/agent-governance/10-dispatch.md`",
                "```\n- `~/Developer/agent-governance/10-dispatch.md`\n```",
            )
        self.assertEqual(self.run_repository(), 1)
    ...
    def test_reference_readme_router_and_skill_links_are_checked(self):
        for name in (checker.RUNTIME_DOC, "docs/reference/new-guide.md", "README.md", checker.ROUTERS[0], checker.SYNC_SKILL):
            with self.subTest(name=name):
                path = self.root / name
    ...
    def test_local_broken_deployment_warns(self):
        self.checks.deployment(self.root / checker.ROUTERS[0], Path("/virtual/home/.codex/AGENTS.md"))
        self.assertEqual(self.checks.exit_code, 0)
        self.assertIn("WARN 本地部署缺失或斷鏈", self.output())

    def test_copy_and_content_are_independent_results(self):
        source = self.root / checker.ROUTERS[0]
        target = Path("/virtual/home/.codex/AGENTS.md")
        self.files[target] = self.files[source]
        with patch.object(Path, "samefile", return_value=False), patch.object(Path, "is_symlink", return_value=False):
            self.checks.deployment(source, target)
        self.assertIn("副本", self.output())
