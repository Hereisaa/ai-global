# 40 — 制度維護協議

> 適用於全域／專案指令與本目錄的修改。授權與驗收統一見 [20-judgment.md](20-judgment.md)，安全細則見 [50-safety.md](50-safety.md)。

## 授權與範圍

- 使用者要求改良制度、修正規則或已核准具體方案，即可完成該範圍內的修改、同步入口與驗證；不重複要求相同授權。
- 一般工程任務不自動授權改全域制度。發現錯誤事實、失效路由或新教訓，先在回覆記錄；取得制度維護授權後再納入。
- 放寬安全限制、改變使用者偏好或擴大核准範圍，需明確授權。不能把「修正事實」當成變更政策的理由。
- 制度只管理使用者可設定的指令層，不能覆蓋平台 system／developer 指令與權限機制。備份、封存與外部引用資料不是生效指令。

## 可回復的修改

1. 確認目前分支、工作樹及指令檔來源；讀專案根目錄 AGENTS.md／CLAUDE.md（若有）。新分支使用 `claude/<主題>`，不覆蓋其他工作者的變更。
2. 修改重要檔案前確保可回復。已追蹤且乾淨的檔案可用明確 Git 基準版本；未追蹤或已有未提交變更的檔案先備份，不能假定 Git 已保存。
3. 不含秘密的備份可放 `governance/backups/<來源>.<yyyyMMdd-HHmmss>`，避免同名覆蓋；含憑證的設定只可備份到 repo 外、限制存取的本機位置，不輸出內容。
4. 修改規則時同步所有生效摘要與路由；一條政策只在一處完整定義，其他文件引用它。保留既有引用路徑，除非本次明確需要重組。
5. 依 20 的高風險標準獨立審查制度變更，並跑下列檢查；回報行為差異、證據與未驗證的平台。

## 檢查與回報

在 ai-global 根目錄執行：

```bash
python3 setup/check_governance.py
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s setup -p 'test_*.py'
python3 setup/check_governance.py --local
```

- 離線檢查驗證兩個入口、路由及現行相對連結；`--local` 額外核對本機掛載與支援的設定欄位。WARN 需說明，FAIL 需修正；未涵蓋項目不能宣稱通過。
- 工具檢查不證明模型會遵守文字。治理改版另依 [治理驗收情境](../docs/reference/governance-evaluation.md) 做獨立讀回；各平台新對話實測才是行為測試，不能用讀回替代。
- router 以 60 行、單份制度以 300 行為維護預算，超過時檢查是否重複或可按需載入；這不是模型能力的硬上限。
- 保留既有變更紀錄，本次規則變動加一筆日期與摘要。精確 diff 與回復依 Git；乾淨且已追蹤的檔案不用每次另製完整備份。

## 發布與跨機器同步

- 正本在 `~/Developer/GitHub/ai-global`。symlink／Junction 指向目前工作樹，切分支或改檔可能立即影響這台機器後續載入；不能宣稱等待 merge 才生效。
- 本地修改、commit、push 與另一台套用是不同步驟。只有使用者已明確授權發布到特定遠端／分支才 push；「改良」不自動包含對外發布。
- 提交只包含本次檔案，先檢查差異與秘密。不要用無範圍的 `git add .` 收進其他人的變更。
- 對帳先唯讀檢查；要求同步時，確認工作樹可安全更新後使用 `git pull --ff-only`。Windows 複製模式要比對內容，不能只檢查檔案存在。
- 不擅自調整模型／effort、權限或安裝第三方能力來消除警告；先判斷差異是本機選擇還是需修復的設定。

## 教訓與記憶

- 只有重複、可驗證且跨任務有用的教訓才值得成為規則；先寫明觸發情境、證據與下一次動作，避免把一次失敗變成永久禁令。
- 記憶寫入遵守各平台及使用者授權；不因發現教訓就自動修改 memory、專案指令或主工作樹。
- 舊備份保留作回復資料，不讀入日常指令。清理依 50 的可逆流程，不以制度維護為由直接銷毀。

## 變更紀錄
- 2026-07-03 建檔（Fable 5）
- 2026-08-17 兩側同步規則新增「跨機器同步」節：制度與全域設定改由 ai-global git repo 承載（Fable 5，應使用者要求）
- 2026-09-07 統一授權來源、採 Git 基準回復、取消自動回寫與推送，加入結構檢查及行為驗收。
